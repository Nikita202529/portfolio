...
const serviceItemAdventages = document.querySelectorAll('.service-item_advantages');

// показать все блоки
function showAllServiceItems(serviceItem) {
...
function showItem() {
    if (window.innerWidth <= 844) {
        showAllServiceItems(serviceItem);
        showAllServiceItems(serviceItemAdventages);
        return;
    }
    ...
    */
    ...
    function setClickGoals() {
        ...
        for (let selector in data.YM_GOALS.main_page.CLICK) {
            ...
            // ВАЖНО: .on() с делегированными событиями. Читай: 
            // http://qaru.site/questions/19639/click-event-doesnt-work-on-dynamically-generated-elements
    ...
    function setOpenModalGoals() {
        ...
        for (let selector in data.YM_GOALS.main_page.MODAL_SHOW) {
            ...
            $(document).on('shown.bs.modal', selector, function () {
                if (typeof shownGoalsInited[selector] !== 'undefined' 
                        && shownGoalsInited[selector] != null 
    ...
    const forms = {
        ...
        // Форма заявки в section questions
        'popup_form_Typical': {
            successCallback: popupFormSuccess,
            modalSelector: "#ModalLong",
            buttonSelector: ".popup-form__btn",
            fields: [{
                post_name: "name",
                html_name: "user-name_request",
                required: true,
            },
            {
                post_name: "phone",
                html_name: "user-phone_request",
                required: false,
                regex: /^(\+7|7|8)+(([0-9,\s,\-,\(,\)]){10,16})$/, // писал сам
                errorMessage: "телефон заполнен неверно",
            },
            {
                post_name: "way_to_contact",
                html_name: "popup-form-contact_way_request",
                dependence: "user-phone_request",
            },
            {
                post_name: "comments",
                html_name: "comments"
